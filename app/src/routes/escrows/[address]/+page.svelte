<script lang="ts">
  ...
  async function loadEscrowData() {
    ...
    try {
      ...
      // Load email notification data from API
  ...
  async function loadEmailNotification() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/escrow/${escrowAddress}?network=${networkName}`
      );
  ...
  async function handleEnableEmail() {
    ...
    try {
      ...
      const response = await fetch(`/api/escrow/register`, {
        method: 'POST',
        headers: {
    ...
    try {
      ...
      if (receipt.status === 'success') {
        ...
        try {
          ...
          await fetch(`/api/escrow/unregister`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
