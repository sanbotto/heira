...
export async function sendInactivityWarning(
  ...
  mailpaceApiToken?: string,
  mailpaceFromEmail?: string,
): Promise<void> {
  const mailpaceToken =
    mailpaceApiToken ||
    (typeof process !== "undefined" && process.env?.MAILPACE_API_TOKEN);
  const fromEmail =
    mailpaceFromEmail ||
    (typeof process !== "undefined" && process.env?.MAILPACE_FROM_EMAIL) ||
    "noreply@heira.app";
