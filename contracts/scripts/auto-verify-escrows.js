...
async function main() {
    ...
    try {
        ...
        for (const event of events) {
            ...
            try {
                ...
                if (
                    ...
                    if (verifyAll) {
                        console.log(`⚠️  Already verified: ${escrowAddress}`);
                    } else {
                        console.log(`⏭️  Skipping (already verified): ${escrowAddress}`);
                    }
                    skippedCount++;
                } else {
        ...
        if (failedCount > 0) {
            console.log(
                "\n⚠️  Some escrows failed verification. You can try verifying them manually:"
            );
            console.log(
